...
export default function ChatWidgetIframe() {
  ...
  // Écouter les messages du parent (portail)
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // En production, vérifier l'origine pour la sécurité
      // if (event.origin !== 'https://votre-portail.com') return

      if (event.data.type === 'VYBUDDY_WIDGET_INIT') {
        console.log('Widget initialized with options:', event.data.options)
      }
    }
  ...
  // Envoyer des messages au parent si nécessaire
  useEffect(() => {
  ...
  const handleClose = () => {
    // Informer le parent que le widget se ferme
    window.parent.postMessage(
      {
        type: 'VYBUDDY_WIDGET_CLOSE',
      },
      '*'
    )
  }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-900">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-indigo-tropical text-white">
