  ...
  return (
    <>
      ...
      {/* Header avec logo et bouton collapse */}
          <div className={`${isCollapsed ? 'p-2' : 'p-4 lg:p-6'} border-b border-vert-profond-light dark:border-gray-700`}>
        <div className={`flex items-center ${isCollapsed ? 'justify-center flex-col gap-2' : 'gap-3'} mb-2`}>
          {isCollapsed ? (
            // Mode réduit : logo et bouton collapse empilés
            <>
              ...
              {/* Bouton collapse (desktop seulement) - visible en mode réduit */}
              <Tooltip content="Développer" position="right">
                <button
            ...
            // Mode étendu : logo, texte et bouton collapse en ligne
            <>
              <div className="w-10 h-10 rounded-full bg-indigo-tropical flex items-center justify-center flex-shrink-0">
                <svg
                  className="w-6 h-6 text-white"
              ...
              <Tooltip content="Réduire" position="right">
                <button
                  onClick={toggleCollapse}
                  className="hidden lg:flex items-center justify-center w-8 h-8 rounded-lg hover:bg-vert-profond-light dark:hover:bg-gray-800 transition-colors flex-shrink-0"
      ...
      <div className="p-4">
        {isCollapsed ? (
          <Tooltip content="Nouveau chat" position="right">
            <button
              onClick={() => {
                onNewChat()
                setIsMobileOpen(false)
              }}
              className="w-full bg-white dark:bg-gray-800 text-vert-profond dark:text-white font-semibold py-3 px-3 rounded-lg hover:bg-sable dark:hover:bg-gray-700 transition-colors flex items-center justify-center gap-2"
            >
              <svg
          ...
          >
            ...
            <span className="truncate">Nouveau chat</span>
          </button>
        )}
      </div>
              ...
              isCollapsed ? (
                <Tooltip key={chat.id} content={chat.title} position="right">
                  <button
                    ...
                    className={`w-full flex items-center justify-center px-3 py-2 rounded-lg text-sm transition-colors ${
      ...
      <div className="p-4 border-t border-vert-profond-light dark:border-gray-700 space-y-2">
        ...
        {isAdmin && (
          <>
            {isCollapsed ? (
              <Tooltip content="Administration - Feedbacks" position="right">
                <button
                  onClick={() => {
                    router.push('/admin/feedbacks')
                    setIsMobileOpen(false)
                  }}
                  className="w-full flex items-center justify-center px-3 py-2 rounded-lg text-sm text-gray-300 dark:text-gray-400 hover:bg-indigo-600 dark:hover:bg-gray-800 hover:text-white transition-colors"
                >
                  <svg
                    className="w-5 h-5 flex-shrink-0"
                    fill="none"
              ...
              >
                ...
                <span className="truncate">Administration - Feedbacks</span>
              </button>
            )}
            {isCollapsed ? (
              <Tooltip content="Administration - Base de connaissances" position="right">
                <button
                  onClick={() => {
              ...
              >
                ...
                <span className="truncate">Administration - Base de connaissances</span>
              </button>
            )}
          </>
        )}
        {isCollapsed ? (
          <Tooltip content="Support téléphonique" position="right">
            <button
              onClick={() => {
                toast('Bientôt disponible')
                setIsMobileOpen(false)
              }}
              className="w-full flex items-center justify-center px-3 py-2 rounded-lg text-sm text-gray-300 dark:text-gray-400 hover:bg-vert-profond-light dark:hover:bg-gray-800 hover:text-white transition-colors"
            >
              <svg
                className="w-5 h-5 flex-shrink-0"
                fill="none"
          ...
          >
            ...
            <span className="truncate">Support téléphonique</span>
          </button>
        )}
        {isCollapsed ? (
          <Tooltip content="Paramètres" position="right">
            <button
              onClick={() => {
                toast('Bientôt disponible')
                setIsMobileOpen(false)
        ...
        {isCollapsed ? (
          <Tooltip content="Déconnexion" position="right">
            <button 
              ...
              className="w-full flex items-center justify-center px-3 py-2 rounded-lg text-sm text-gray-300 dark:text-gray-400 hover:bg-red-600 dark:hover:bg-red-800 hover:text-white transition-colors"
            >
              <svg
                className="w-5 h-5 flex-shrink-0"
                fill="none"
